...
const help_option = {
  ...
  "--quick":
    "admins personal settings [g++ -DLOCAL -std=c++17 -Wshadow -Wall -o file file.cpp -fsan  itize=address -fsanitize=undefined -D_GLIBCXX_DEBUG -g]"
};
...
function input_handler(line, listen) {
  ...
  //Checking for input option
    ...
    if (fs.existsSync(input_file)) {
      ...
      execute(command.compile_command, command.exec_command, command.file_name);
...
function listener() {
  const listen = readline.createInterface({
    input: process.stdin,
