...
const option = {
    ...
    '--check' : 'user setting g++ -DLOCAL -std=c++17 -Wshadow -Wall -o file file.cpp -fsanitize=address -fsanitize=undefined -D_GLIBCXX_DEBUG -g'
}

//const map = new Map( [ ['Wall', '-Wall'], ['Werror', '-Werror'], [], [], [], [], []  ] );
...
function checker ( args ) {
    ...
    if ( parse_object.check ) {
        ...
        for ( let key in parse_object ) {
            ...
            if ( option[`--${key}`] ) {
                flag_string += ` -${key}${ (typeof parse_object[key] !== 'boolean'? option[key]: '') }`;  
            } 
        }
    }
