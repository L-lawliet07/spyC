...
const help_option = {
  ...
  "--quick":
    "admins personal settings [g++ -DLOCAL -std=c++17 -Wshadow -Wall -o file file.cpp -fsanitize=address -fsanitize=undefined -D_GLIBCXX_DEBUG -g]"
};
...
function checker(args) {
  ...
  if (parse_object.check) {
    flag_string =
      " -DLOCAL -std=c++17 -Wshadow -Wall -fsanitize=address -fsanitize=undefined -D_GLIBCXX_DEBUG -g";
  } else {
...
module.exports = {
  parser: args => {
    ...
    if (parse_object.i || parse_object.input) {
      exec_command = `${exec_command} < ${path.join(
        cwd,
        parse_object.i || parse_object.input
      )}`;
    }
